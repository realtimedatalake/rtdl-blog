...
few years of my career working in tech for companies that mad data tools. I’d become familiar with 
the tool ecosystem of and common use cases across a data stack. Yet, I couldn’t figure out why 
real-time data – more specifically, near real-time data, with latencies in the seconds – was so 
difficult.  
  
Databricks is making data lakes and the lakehouse architecture popular, but even their real-time 
solutions take a lot of work and frequently can’t get latencies down to seconds or a minute.  
...
you can just plug a Kafka stream or a webhook from Segment into that will automatically build 
you a real-time data lake?  
...
## How does rtdl work?
You configure a record in rtdl. A stream includes authentication information for the data store 
your data lake will be built in, data lake configuration options, and identifiers - an id and 
an alternate id - to be included in data payloads sent to rtdl. Once a stream is configured, any 
JSON payload sent to rtdl that includes the same id or alternate id as the stream will be processed 
into Parquet and stored in the data store and as defined in the stream.  
